...
public class ChatMessageUnitTests {
  ...
  public void whenSearchingForRecipientAndSenderAndSendingMessage_thenReturnChatMessage() {
    ...
    ChatMessageDto chatMessage =
        chatMessageService.createAndSaveMessageForPrivateUser(payload, principal, user2.getEmail());
  ...
  public void whenSearchingForRecipientAndSenderAndSendingMessageSenderNotFound_thenReturnUserNotFound() {
    ...
    Assertions.assertThrows(UserNotFoundException.class, () -> chatMessageService
        .createAndSaveMessageForPrivateUser(payload, principal, user1.getEmail()));
  ...
  public void whenSearchingForRecipientAndSenderAndSendingMessageRecipientNotFound_thenReturnUserNotFound() {
    ...
    Assertions.assertThrows(UserNotFoundException.class, () -> chatMessageService
        .createAndSaveMessageForPrivateUser(payload, principal, user2.getEmail()));
    verify(chatMessageRepository, never()).save(any(ChatMessageEntity.class));
  }
  ...
  public void whenPullingMessageHistory_thenReturnChatMessageHistory() {
    ...
    ChatMessageEntity messageEntity1 = ChatMessageEntity.builder().sender(user2).recipient(user1)
        .content("Hey its me!").timestamp(LocalDateTime.now().minusMinutes(5)).build();

    ChatMessageEntity messageEntity2 = ChatMessageEntity.builder().sender(user1).recipient(user2)
        .content("Do you want my car?").timestamp(LocalDateTime.now().minusMinutes(4)).build();
  ...
  public void getInbox_HappyPath_PartnerIsSender() {
    ...
    ChatMessageEntity lastMsg =
        ChatMessageEntity.builder().sender(partner).recipient(me).content("Cześć Adrian!").build();
  ...
  void markMessagesAsRead_DatabaseError() {
    ...
    doThrow(new RuntimeException("Database connection lost")).when(chatMessageRepository)
        .markAllAsRead(sender, recipient);
