...
public class UsersFinderUnitTests {
  ...
  void allUsers_shouldThrowRuntimeException_whenRepositoryFails() {
    ...
    RuntimeException thrown = assertThrows(RuntimeException.class, () -> usersFinder.allUsers(),
  ...
  void getLoggedInUser_shouldReturnMappedDto_whenUserIsFound() {
    ...
    UserDashboardResponseDto expectedDto = new UserDashboardResponseDto("test@test.com", 3, 2,
        "ROLE_USER", UUID.randomUUID(), mockEntity.getName());
  ...
  void getLoggedInUser_shouldThrowException_whenMappingFails() {
    ...
    when(userDashboardResponseMapper.toDto(any()))
        .thenThrow(new RuntimeException("Mapping failure"));

    assertThrows(RuntimeException.class, () -> usersFinder.getLoggedInUser(mockEntity.getEmail()),
