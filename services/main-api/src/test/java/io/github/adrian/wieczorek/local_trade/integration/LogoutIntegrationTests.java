...
public class LogoutIntegrationTests extends AbstractIntegrationTest {
  ...
  void shouldBlacklistAccessTokenAfterLogout() throws Exception {
    ...
    assertNotNull(accessTokenCookie,
        "Ciasteczko 'token' nie zostało znalezione w odpowiedzi logowania.");
    String accessToken = accessTokenCookie.getValue();

    Cookie refreshTokenCookie = loginResult.getResponse().getCookie("refreshToken");
    assertNotNull(refreshTokenCookie,
        "Ciasteczko 'refreshToken' nie zostało znalezione w odpowiedzi logowania.");

    mockMvc.perform(get("/favorite/me").header("Authorization", "Bearer " + accessToken))
        .andExpect(status().isOk());

    mockMvc.perform(post("/auth/logout").header("Authorization", "Bearer " + accessToken)
        .cookie(refreshTokenCookie)).andExpect(status().isOk());
